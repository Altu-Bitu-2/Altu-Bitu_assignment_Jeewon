...
using namespace std;
typedef pair<int, int> ci;	//시작시간과 종료시간을 저장하는 수업 자료구조 
...
int main() {
    ...
    //입력
    cin >> n;	//수업 개수 입력 받기 
    vector<ci> lec(n, ci(0, 0));	//수업 배열 
    for (int i = 0; i < n; i++) {	//n개의 수업에 대한 정보 입력 받기 
        cin >> lec[i].first >> lec[i].second; //first: 시작시간, second: 종료시간
    }

    //연산
    sort(lec.begin(), lec.end());	//수업 오름차순  정렬 
